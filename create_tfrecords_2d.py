...
class ParticleOracle2D(py_policy.PyPolicy):
    """Creates Policy to generate training data"""
    def __init__(self, env, goal_threshold=0.01):
        super(ParticleOracle2D, self).__init__(env.time_step_spec(), env.action_spec())
        self._env = env
...
def create_episodes(dataset_path, num_episodes):
    ...
    env = suite_gym.load('Particle2D-v1')
    policy = ParticleOracle2D(env)
...
def create_video():
    ...
    for seed in seeds:
        ...
        particle_policy = ParticleOracle2D(env)

        video_path = os.path.join('data', 'videos_2d', 'ttl=7d', 'vid_%d.mp4' % seed)
...
def main(_):
    if FLAGS.make_video:
        ...
        dataset_path = 'data/2d_oracle_particle.tfrecord'
