...
class Particle2d(gym.Env):
    ...
    def _create_observation_space(self):
        obs_dict = collections.OrderedDict(
            pos_agent=spaces.Box(low=-2., high=2., shape=(2,), dtype=np.float32),
            vel_agent=spaces.Box(low=-1e2, high=1e2, shape=(2,), dtype=np.float32),
            pos_goal=spaces.Box(low=-2., high=2., shape=(2,), dtype=np.float32)
        )
    ...
    def reset(self):
        ...
        obs = dict()
        obs['pos_agent'] = self._rng.uniform(low=-2., high=2., size=2).astype(np.float32)
        obs['vel_agent'] = np.array([0., 0.])
        obs['pos_goal'] = self._rng.uniform(low=-2., high=2., size=2).astype(np.float32)
    ...
    def _internal_step(self, action, new_action):
        ...
        # # xdot_{desired} is always (0, 0) --> u = k_p (x_{desired} - x) - k_v (xdot)
    ...
    def render(self, mode='rgb_array'):
        fig, _ = visualize_2d(self.obs_log, self.act_log)
...
def visualize_2d(
    ...
    ax.add_patch(plt.Circle((pos_goal[0], pos_goal[1]), 0.01, color='g'))
    ...
    for i in range(len(obs_log_) - 1):
        ...
        ax.plot(pos_agent_2step[:, 0], pos_agent_2step[:, 1], alpha=alpha, linestyle=':', color='black')
    if last_big:
        ax.scatter(obs_log_[-1]['pos_agent'][0], obs_log_[-1]['pos_agent'][1], marker='o', s=50, color='black')
...
if 'Particle2D-v1' in registration.registry.env_specs:
    del registration.registry.env_specs['Particle2D-v1']

registration.register(id='Particle2D-v1', entry_point=Particle2d)
